...
export class HomeDirectoryComponent implements OnInit {
  loading: boolean = false;

  colHeaderCharity: any = [];
  listCharity: any;

  activeIndexDirectory = 0;
  ...
  applyFilterGlobal() {
    if (this.activeIndexDirectory == 0) {
  ...
  goToDetailCharity(Id) {
    this.router.navigate(['/home-dashboard', { type: this.encrDecrService.set(4), objectId: this.encrDecrService.set(Id) }])
  }

  goToDetailCompany(Id) {
    this.router.navigate(['/home-dashboard', { type: this.encrDecrService.set(5), objectId: this.encrDecrService.set(Id) }])
  }
